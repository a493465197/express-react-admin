...
const getMessages = async (request, response) => {
    ...
    if (user) {
        ...
        const messages = await models.Message.find({conversation: id}).sort({_id:-1}).limit(10)
                .populate('creator')
                .sort('timestamp')
...
const like = async (request, response) => {
    ...
    if (user) { 
        ...
        const message = await models.Message.findOne({_id: msgid})
        if (message.creator.toString() === user.toString()) {
            const result = await models.Message.updateOne({_id: msgid},{like: (message.like || 0)+1})
            if (result.acknowledged) {
                response.json({status: 'success'})
            } else {
                response.json({'status': 'unable to delete message'})
...
const dislike = async (request, response) => {
    ...
    if (user) { 
        ...
        const message = await models.Message.findOne({_id: msgid})
        if (message.creator.toString() === user.toString()) {
            const result = await models.Message.updateOne({_id: msgid},{ dislike: (message.dislike || 0)+1})
            if (result.acknowledged) {
                response.json({status: 'success'})
            } else {
                response.json({'status': 'unable to delete message'})
